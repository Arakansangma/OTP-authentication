  ...
  function onSignup() {
    ...
    signInWithPhoneNumber(auth, formatPh, appVerifier)
      ...
      .catch((error) => {
        console.log(error);
        setLoading(false);
        toast.error("ERROR: Check the entered phone number")
      });
        ...
        {user ? (
          ...
          <div className="w-80 flex flex-col gap-4 rounded-lg p-4">
            <h1 className="text-center leading-normal text-white font-medium text-3xl mb-6">
              OTP <br /> authentication tester
            </h1>
            {showOTP ? (
              <>
                <div className="bg-white text-gray-900 w-fit mx-auto p-4 rounded-full">
              ...
              <>
                ...
                <button
                  onClick={onSignup}
                  className="bg-gray-800 w-full flex gap-1 items-center justify-center py-2.5 text-white rounded"
                >
                  {loading && (
                    <CgSpinner size={20} className="mt-1 animate-spin" />
                  )}
